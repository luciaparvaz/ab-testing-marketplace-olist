"""Unit tests for the synthetic treatment effect (diluted model)."""
import numpy as np
import pytest
...
def data():
    ...
    values = rng.lognormal(mean=4.5, sigma=0.9, size=200_000)   # ~ like the AOV
...
def test_average_treatment_effect_matches_ate(data):
    ...
    # declared ATE = 5%; wide margin for sampling noise
...
def test_effect_is_multiplicative_and_positive_on_responders(data):
    ...
    # among responders the mean factor ~ 1 + delta_resp
