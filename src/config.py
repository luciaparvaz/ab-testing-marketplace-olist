...
Carga `params.yaml` (única fuente de verdad para los parámetros científicos) y añade las rutas
y los valores derivados. **Todos los demás módulos importan de aquí; ninguno define constantes.**

Las rutas son absolutas (derivadas de la ubicación de este fichero), así que los scripts, el
notebook y los tests funcionan desde cualquier directorio de trabajo.
"""
...
try:
    sys.stdout.reconfigure(encoding="utf-8")   # Windows abre stdout en cp1252 por defecto
except Exception:
    pass
...
ATE: float = P_RESP * DELTA_RESP                     # derivado: efecto medio (+5 %)
...
def summary() -> str:
    return (f"SEED={SEED} ALPHA={ALPHA} | efecto diluido p_resp={P_RESP} delta_resp={DELTA_RESP} "
            f"-> ATE={ATE:.2%} | MDE={MDE_RELEVANCIA}% | ventana {WINDOW_START}..{WINDOW_END} | "
