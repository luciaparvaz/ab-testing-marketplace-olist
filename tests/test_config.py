"""Sanity checks sobre la configuración (params.yaml -> config.py)."""
import config
...
def test_window_is_20_months():
    ...
    # ventana exclusiva en 2018-09-01 -> 20 meses
    import pandas as pd
...
def test_paths_exist_and_are_absolute():
    ...
    # PROC / OUT_* se crean al importar config
...
def test_no_module_redefines_constants():
    """Ningún otro módulo debe volver a definir SEED/ALPHA/... a nivel de módulo."""
    import ast
    ...
    for py in src.glob("*.py"):
        ...
        for node in tree.body:  # solo asignaciones a nivel de módulo
            if isinstance(node, ast.Assign):
                for tgt in node.targets:
                    if isinstance(tgt, ast.Name) and tgt.id in forbidden:
                        offenders.append(f"{py.name}:{tgt.id}")
    assert not offenders, f"constantes redefinidas fuera de config.py: {offenders}"
